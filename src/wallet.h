...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 XORN at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 1000                          // UTXO's that are valid for staking
};

// Possible states for zMRJA send
enum ZerocoinSpendStatus {
    ZXORN_SPEND_OKAY = 0,                            // No error
    ZXORN_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZXORN_WALLET_LOCKED = 2,                         // Wallet was locked
    ZXORN_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZXORN_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZXORN_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZXORN_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZXORN_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZXORN_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZXORN_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZXORN_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZXORN_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZXORN_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZXORN_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZXORN_SPENT_USED_ZXORN = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZXORNAutoBackups(bool fEnabled)
    {
