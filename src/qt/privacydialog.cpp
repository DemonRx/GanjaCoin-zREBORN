...
#include "zganjacoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zMRJA ought to be enough for anybody." - Bill Gates, 2017
    ui->zXORNpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzXORNSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzXORN->setEnabled(false);
        ui->pushButtonMintzXORN->setToolTip(tr("zMRJA is currently disabled due to maintenance."));

        ui->pushButtonSpendzXORN->setEnabled(false);
        ui->pushButtonSpendzXORN->setToolTip(tr("zMRJA is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zXORNpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzXORN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMRJA is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMRJA...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMRJA in ") +
...
void PrivacyDialog::on_pushButtonSpendzXORN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMRJA is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMRJA
        sendzXORN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMRJA
    sendzXORN();
}

void PrivacyDialog::on_pushButtonZXORNControl_clicked()
{
    ZXORNControlDialog* zXORNControl = new ZXORNControlDialog(this);
    zXORNControl->setModel(walletModel);
    zXORNControl->exec();
}

void PrivacyDialog::setZXORNControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzXORNSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzXORN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid MRJA Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zXORNpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMRJA is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MRJA </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMRJA</b>";
    ...
    // use mints from zMRJA selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZXORNControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZXORNControlDialog::GetSelectedMints();
    }
    ...
    // Clear zMRJA selector in case it was used
    ZXORNControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMRJA Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " MRJA, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMRJA Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMRJA </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMRJA "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMRJA "));
    ui->labelzXORNAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzXORNSyncStatus->setVisible(fShow);
}
