                                                    ...
                                                    fFeeMinimized(true)
{
    ...
    if (!settings.contains("bUseSwiftNode"))
        settings.setValue("bUseSwiftNode", false);

    bool useSwiftNode = settings.value("bUseSwiftNode").toBool();
    if (fLiteMode) {
        ui->checkSwiftNode->setVisible(false);
        CoinControlDialog::coinControl->useObfuScation = false;
        CoinControlDialog::coinControl->useSwiftNode = false;
    } else {
        ui->checkSwiftNode->setChecked(useSwiftNode);
        CoinControlDialog::coinControl->useSwiftNode = useSwiftNode;
    }

    connect(ui->checkSwiftNode, SIGNAL(stateChanged(int)), this, SLOT(updateSwiftNode()));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (ui->checkSwiftNode->isChecked()) {
        recipients[0].useSwiftNode = true;
        strFunds += " ";
        strFunds += tr("and SwiftX");
    } else {
        recipients[0].useSwiftNode = false;
    }
...
void SendCoinsDialog::updateSwiftNode()
{
    QSettings settings;
    settings.setValue("bUseSwiftNode", ui->checkSwiftNode->isChecked());
    CoinControlDialog::coinControl->useSwiftNode = ui->checkSwiftNode->isChecked();
    coinControlUpdateLabels();
}
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid MRJA address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
