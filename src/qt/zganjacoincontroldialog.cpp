...
// Copyright (c) 2017-2018 The ganjacoin developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zganjacoincontroldialog.h"
#include "ui_zganjacoincontroldialog.h"
...
std::list<std::string> ZXORNControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZXORNControlDialog::listMints;

ZXORNControlDialog::ZXORNControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZXORNControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZXORNControlDialog::~ZXORNControlDialog()
{
    delete ui;
}

void ZXORNControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZXORNControlDialog::updateList()
{
...
void ZXORNControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZXORNControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZXORN_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZXORNControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZXORNControlDialog::GetSelectedMints()
{
...
void ZXORNControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
