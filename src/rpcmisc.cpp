...
// Copyright (c) 2017-2018 The XORN developers
...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total xorn balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zMRJAsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zMRJA denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zMRJA denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zMRJA denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zMRJA denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zMRJA denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zMRJA denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zMRJA denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zMRJA denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zMRJA denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in xorn/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in xorn/kb\n"
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zxornObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zxornObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zMRJAsupply", zxornObj));
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"xornaddress\"\n"
            "\nReturn information about the given xorn address.\n"
            "\nArguments:\n"
            "1. \"xornaddress\"     (string, required) The xorn address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"xornaddress\", (string) The xorn address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: XORN address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are xorn addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) xorn address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"xornaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"xornaddress\"  (string, required) The xorn address to use for the signature.\n"
