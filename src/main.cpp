...
// Copyright (c) 2017 The MRJA developers
...
#if defined(NDEBUG)
#error "MRJA cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zMRJA spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zMRJA spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zMRJA spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if (nHeight == 0) {
		 ...
		 ret = blockValue  / 100 * 75; // 2% to the devfee
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from XORN
		...
		// erase the spent input - fake POS fix
...
{
    RenameThread("MRJA-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZXORNMinted()
{
...
void RecalculateZXORNSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zMRJA supply
...
bool RecalculateXORNSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zMRJA with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zMRJASpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MRJA to zMRJA
	...
	bool found = false;
        BOOST_FOREACH (CTxOut out, txNew.vout) {
            ...
            if (devRewardscriptPubKey == out.scriptPubKey) {
                ...
                CAmount devFee = blockValue * 0.03;
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zMRJA spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zMRJA serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zMRJA mints and %d zMRJA spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // MRJA: We use certain sporks during IBD, so check to see if they are
...
bool IsPoSFixActive()
{
	return IsSporkActive(SPORK_17_FAKE_STAKE_FIX);
}

int ActiveProtocol()
{
	int nForkHeight = GetForkHeight();
	int nCurHeight = chainActive.Height(); // unblocking version
	
    if (nCurHeight >= nForkHeight-15) {
		if (IsPoSFixActive())
			return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT_POSFIX;
		else
			return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;
    }
    else
